...
//PWM settings
const int freq = 250;       // PWM frequency in Hz (50Hz servo, 250Hz common ESC, 400Hz also common ESC)
const int resolution = 14;  // 14-bit PWM resolution
//Loop time (ms)
double loopTime = 1000000 / freq;  //Loop time in micros
//Pid coefficients
double Kp = 2;         //Proportial part of pid controller
double Ki = 3.5 / freq;  //contribution from integrating part of controller
...
void setPulseWidth(int pin, int pulseWidth) {
  int dutyCycle = (pulseWidth / loopTime) * 16384;  // Convert microseconds to duty cycle when resolution is 14 bit (16384) and frequency 250hz (4000us)
...
void setup() {
  ...
  accelgyro.setDLPFMode(0x02);  // BandWidth: 6=5Hz, 5=10Hz, 4=20Hz, 3=40Hz, 2=100Hz, 1=180Hz, 0=256Hz
...
void loop() {
  ...
  if (last_update + loopTime <= micros()) {
    ...
    //PID
    y_x = (double)gx / 16.4;  // degrees/s
    e_x = ref_x - y_x;
    e_x_prev = e_x;
    D_x = (e_x - e_x_prev);
    I_x = I_x + Ki * (e_x + e_x_prev) / 2;
    PDIFF_x = Kp * e_x + I_x + Kd * D_x;


    y_y = (double)gy / 16.4;  // degrees/s
    e_y = ref_y - y_y;
    e_y_prev = e_y;
    D_y = (e_y - e_y_prev);
    I_y = I_y + Ki * (e_y + e_y_prev) / 2;

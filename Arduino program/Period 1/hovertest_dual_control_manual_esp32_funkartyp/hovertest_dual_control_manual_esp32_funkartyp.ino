...
//Pid coefficients
double Kp = 350;                 //Proportial part of pid controller
double Ki =9;                //contribution from integrating part of controller
double Kd = 0/0.001;            //Denna är så stor så att vi inte måste dividera med tiden (1ms) för varje iteration av loopen
//Pid variables
...
void setup() {
  ...
  {
    Serial.println("PWM setup sucsessful");
  }
  else
  {
...
void loop() {
  if(last_update + loopTime <= millis()) //Vi uppdaterar varannan ms
  {
    ...
    if(digitalRead(switchPin)) //Force refrence to 0
    {
      ref_y = 0.00;    
      ref_x = 0.00;    
    }
    ...
    {
      ref_x = map(analogRead(xrefPot), 0, 4095.0, -100, 100)/10000.0; //Map funktionen klarar inte decimaltal så vi delar med 10e3 för -1 till 1 range
      ref_y = map(analogRead(yrefPot), 0, 4095.0, -100, 100)/10000.0;   
    }

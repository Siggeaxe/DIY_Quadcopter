...
//Pid coefficients
double Kp = 300;                 //Proportial part of pid controller
double Ki =2250/freq;                //contribution from integrating part of controller
...
void loop() {
  if(last_update + loopTime <= micros()) //Vi uppdaterar varannan ms
  {
    ...
    {
      ref_x = 0;// map(analogRead(xrefPot), 0, 4095.0, -100, 100)/10000.0; //Map funktionen klarar inte decimaltal så vi delar med 10e3 för -1 till 1 range
      ref_y = 0; //map(analogRead(yrefPot), 0, 4095.0, -100, 100)/10000.0;   
    }
    //printCool();
    plotCool();
